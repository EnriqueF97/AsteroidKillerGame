...
shootSpeed = 0.05, // less is faster shooting
bulletDamage = 20, // damage from every bullet to asteroid
gunHealth = 20, // players health
asteroidDamage = 1, // how much an asteroid hit will deal
cannonRotationSpeed = 1.3, // more is faster rotation
scorePerAsteroidDestroyed = 100, // score per asteroid destroyed (lol)
...
let movingRight = false, movingLeft = false, movingUp = false, movingDown = false, firing = false, slowMoving = false;
...
let material = null;
let SHADOW_MAP_WIDTH = 256, SHADOW_MAP_HEIGHT = 256;
...
function createScene(canvas)
{
    ...
    audioLoader.load( '../sounds/asteroidExplosion.wav', function( buffer ) { // cannon sound while shooting
    ...
    audioLoader.load( '../sounds/asteroidImpact.ogg', function( buffer ) { // asteroid hitting the upper platform
        asteroidImpactSound.setBuffer( buffer );
        asteroidImpactSound.setLoop( false );
        asteroidImpactSound.setVolume( 1 );
    });
    audioLoader.load( '../sounds/hitOnAsteroid.wav', function( buffer ) { // hit on asteroid
    ...
    audioLoader.load( '../sounds/asteroidDestruction.wav', function( buffer ) { // destruction of an asteroid
        asteroidDestructionSound.setBuffer( buffer );
        asteroidDestructionSound.setLoop( false );
        asteroidDestructionSound.setVolume( 1.5 );
    });
    audioLoader.load( '../sounds/hitOnAsteroid.wav', function( buffer ) { // asteroid hitting the lower asteroid rock formation
...
// this is used for debugging, spawns an asteroid on the given coordenates
function createAsteroidFrom(x, y, z){
    ...
    // collision box
    asteroid.box = new THREE.Box3().setFromObject(asteroid);
    asteroid.bbox = new THREE.BoxHelper(asteroid, 0x00ff00);
...
function animate() 
{
    ...
    if (!gameOver && !pause){
        ...
        // gun rotation
        if (movingUp){
            if (sphereGun.rotation.x < 4.50){ 
                if (!slowMoving){sphereGun.rotation.x += angle * cannonRotationSpeed;}
                if (slowMoving){sphereGun.rotation.x += angle * cannonRotationSpeed / 4;}
            }
        } 
        if (movingDown){
        ...
        if (movingLeft){
            if (!slowMoving){movingGun.rotation.y += angle * cannonRotationSpeed;}
            if (slowMoving){movingGun.rotation.y += angle * cannonRotationSpeed / 4;}
        }
        if (movingRight){
            if (!slowMoving){movingGun.rotation.y -= angle * cannonRotationSpeed;}
            if (slowMoving){movingGun.rotation.y -= angle * cannonRotationSpeed / 4;}
        ...
        if (waitForNewAsteroid > asteroidsDifficulty){ 
            createAsteroid(); waitForNewAsteroid = 0; 
        ...
        //removing of asteroids who had lived a very long and happy life
        asteroidsArray.forEach(as => {
        ...
        bulletsArray.forEach(bullet =>{
            ...
            // movement
            bullet.position.z += bullet.shootZ*2;
            bullet.position.y += bullet.shootY*2;
            bullet.position.x += bullet.shootX*2;
            bullet.box = new THREE.Box3().setFromObject(bullet);
            bullet.bbox = new THREE.BoxHelper(bullet, 0x00ff00);
        ...
        asteroidsArray.forEach(as => {
            ...
            if (platformBox.intersectsBox(as.box)){
                ...
                if (platform.health <= 0) {gameOver = true; screenShake.shake( camera, new THREE.Vector3(0, 1.5, 0), 1000 /* ms */ );}
                // strong camera shake
                if (screenShake.enabled == false) {
...
document.addEventListener('keydown', function (event) {
    ...
    if (event.keyCode === 16) { // slow aiming
        event.preventDefault();
        slowMoving = true;
        camera.zoom = 3;
        camera.rotation.x = Math.PI / -80;
        crosshair.position.set(0, 0.35, -10);
...
document.addEventListener('keyup', function (event) {
    ...
    if (event.keyCode === 16) { // fast aiming
        event.preventDefault();
        slowMoving = false;
        camera.zoom = 1;
        camera.rotation.x = Math.PI / -25;
...
function modelCreation(){
    ...
    // bullet
    geometry = new THREE.DodecahedronGeometry(0.10, 0);
